...
import { isEmpty, forEach, get, set, reduce } from "lodash";
  ...
  methods: {
    saveData(values) {
      ...
      if (isEmpty(this.entryData)) {
        ...
        this.$q.loading.show();
        this.$axios
          .post(getUrl("storeQuestion"), createData)
          .then(({ data: { question } }) => {
            this.$q.loading.hide();
        ...
        if (!isEmpty(updateData)) {
          ...
          this.$q.loading.show();
          this.$axios
            .post(getUrl("updateQuestion"), updateData)
            .then(({ data: { question } }) => {
              this.$q.loading.hide();
