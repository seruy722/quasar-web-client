...
export default {
  ...
  computed: {
    ...
    generalCargoData() {
      return (
        get(this.$store.getters["cargoDebts/getGeneralData"], "cargo") || {
          kg: 0,
          sale: 0,
          place: 0,
          sum: 0,
    ...
    generalDebtsData() {
      return (
        get(this.$store.getters["cargoDebts/getGeneralData"], "debts") || {
          sum: 0,
          commission: 0,
          visible: true,
        }
  ...
  methods: {
    ...
    getClientData(clientId) {
      this.loading = true;
      this.$store
        .dispatch("cargoDebts/getCargoDebts", clientId)
        .then(() => {
          this.loading = false;
        })
        .catch(() => {
    ...
    async refresh(done) {
      ...
      this.$store.dispatch("cargoDebts/getGeneralData");
      this.$store
        .dispatch("cargoDebts/getCargoDebts", this.currentCodeClientId)
        .then(() => {
          callFunction(done);
          this.$q.loading.hide();
