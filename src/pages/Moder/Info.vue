<template>
  <q-page data-vue-component-name="InfoCodesActivity">
    <q-card>
      <q-card-section>
                ...
                <q-list separator dense>
                  ...
                  >
                    ...
                    <q-item-section side>
                      ...
                      <q-item-label v-else lines="5">
                        {{ col.value }}
                      </q-item-label>
                    </q-item-section>
                  </q-item>
                </q-list>
                <div class="text-center text-bold">Все комментарии</div>
                <q-input
                  v-model="comment"
              ...
              <q-td colspan="100%">
                <div class="q-mb-lg">
                  ...
                  <q-btn
                    ...
                    dense
                    @click="
                      addComment(props.row.id, comment, props.row.comments)
                    "
                  />
                </div>
                  ...
                  >
                    ...
                    <q-item-section side>
                      <q-btn
                        ...
                        dense
                        @click="
                          removeCodesComments(item.id, props.row.comments)
                        "
                      />
                    </q-item-section>
                  </q-item>
...
import { findIndex, map, get, first, orderBy } from "lodash";
...
export default {
  ...
  methods: {
    removeCodesComments(id, comments) {
      this.$axios.get(`${getUrl("removeCodesComments")}/${id}`).then(() => {
    ...
    fetch() {
      this.loading = true;
      this.$axios
        .get(getUrl("statisticsForCodes"))
        .then(({ data: { general } }) => {
          const arr = map(general, (item) => {
            item.last_comment = get(first(item.comments), "comment");
            return item;
          });
          this.infoTableData.data = orderBy(arr, ["index"], ["asc"]);
          this.loading = false;
        })
        .catch(() => {
          this.loading = false;
